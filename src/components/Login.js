...
import rocket_logo from "../assests/img/rocket_logo2.png"
   ...
   function go_to_home(){
    ...
    async function postData(data){
        ...
        const response=await fetch("https://accounts.spotify.com/api/token",{
            ...
            body:"grant_type=client_credentials&client_id="+data.email+"&client_secret="+data.password
        });
        ...
        if(response.status === 200){
                    ...
                    console.log(token);
                    localStorage.setItem("spotify_x-auth-token",token);
    ...
    <div>
      <Formik
        ...
        validate={values => {
          ...
          else if (!/^[A-Z0-9]/i.test(values.email)) {
            errors.email = 'Invalid client ID';
          }
          if (!values.password) {
            errors.password = 'Required';
          }
          else if(values.password.length<30){
            errors.password = 'The minimum secret key length is 30 characters';
          }
          return errors;
        }}
        ...
        {({ values,errors,touched,handleChange,handleBlur,handleSubmit,isSubmitting}) => (
          <form onSubmit={handleSubmit} className="flex flex-col">
            <input
              ...
              className="bg-[#3C434E] mt-[5%]  md:w-[27%] md:mt-[2.8%] sm:w-[45%] sm:mt-[3.5%] w-[50%] p-2 ml-auto mr-auto text-sm font-light text-white rounded-md focus:outline-none focus:bg-[#4C525F]"
            />
...
const Login=()=>{
   ...
   <span className="text-5xl mr-auto ml-auto mt-[20%] md:mt-[13%] md:text-6xl sm:mt-[17%] font-extralight text-[#FCFCFC] hover:cursor-pointer ">Spotify but better!!</span>
        <Basic/>
